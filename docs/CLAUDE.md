...
Actions; if you change plotting code, run
[`devtools::test()`](https://devtools.r-lib.org/reference/test.html)
locally with vdiffr enabled and inspect `tests/testthat/_snaps/` before
committing.
...
— `NAMESPACE` is generated and shouldn’t be edited by hand.
...
**Running checks.** The maintainer prefers to run
[`devtools::test()`](https://devtools.r-lib.org/reference/test.html) /
`document()` / `check()` and
[`pkgdown::build_site()`](https://pkgdown.r-lib.org/reference/build_site.html)
...
`just site`). Lightweight read-only sanity
