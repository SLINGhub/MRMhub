...
> run (pre-C+I). Remaining: H (arg-validation), then re-run
> `document()`.** - **C+I done (`de0f0ad`)**: ~320 `col_*()` wraps
> relocated to `mh_*()`; nested
> [`glue::glue()`](https://glue.tidyverse.org/reference/glue.html)
...
once. `air format` on touched files as you go is fine (fast). Do NOT run
the full suite after every individual edit.

## Verification (end-to-end)

1.  **Static:**
    [`devtools::load_all()`](https://devtools.r-lib.org/reference/load_all.html)
    parses cleanly; grep confirms zero remaining
    `col_red(`/`col_yellow(`/`col_green(` wraps and zero `glue::glue(`
    nested in cli.
...
9.  **Tests (ask the maintainer to run):** `just test` /
    [`devtools::test()`](https://devtools.r-lib.org/reference/test.html)
    green after test text updates; then
    `git checkout -- tests/testthat/_snaps` per the hazard note.
10. **Ask the maintainer** to run
    [`devtools::document()`](https://devtools.r-lib.org/reference/document.html)
    (new exports / the 2 new dataset columns) and
