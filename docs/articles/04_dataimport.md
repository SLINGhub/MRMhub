...
| Generic wide CSV | [`import_data_csv_wide()`](https://slinghub.github.io/MRMhub/quant/reference/import_data_csv_wide.md) | Samples/analyses as rows and features as columns. Can contain columns specific sample annotations. | .csv |
| Generic long CSV | [`import_data_csv_long()`](https://slinghub.github.io/MRMhub/quant/reference/import_data_csv_long.md) | Long-format table with each row being a unique `analysis_id` and `feature_id` pair and addtional columns representing feature variables and sample/method information | .csv |
...
the `MRMhubExperiment`. To include available metadata, set the argument
`import_metadata = TRUE`.

## MRMhub

Output files from
[MRMhub](https://github.com/MRMhub/MRMhub "https://github.com/MRMhub/MRMhub"),
an open-source peak integration software for MRM data () can imported
directly. Specific metadata present in the data file can be imported as
well (\`import_metadata = TRUE\`)
...
Small molecule peak integration results from [Skyline](#id_0) can be
