...
The **`MRMhubExperiment`** object is the **primary data container** in the MRMhub workflow. 
It holds all the experimental and processed data and metadata, as well as details 
of the applied processing steps and the current status of the data. Most `MRMhub` functions take the `MRMhubExperiment` object as data input. Functions that 
process the data return an updated `MRMhubExperiment` object, which can then be 
used in subsequent steps.

Data within the `MRMhubExperiment` is organized into
