...
+==============+=========================+==============+
| mzML_files   | Location of mzML files  | mzML         |
|              | to be used for peak     |              |
|              | integration             |              |
+--------------+-------------------------+--------------+
| batch_info   | Input file with mzML    | b            |
|              | file names and meta     | a            |
|              | information (see below) | tch_info.csv |
+--------------+-------------------------+--------------+
| tran         | Input file with MRM     | fea          |
| sition_list  | transition information  | t            |
|              | (see below)             | ure_info.csv |
+--------------+-------------------------+--------------+
| peak_width   | \[w, x, y, z\] define   | \[0.17,      |
|              | allowed integration     | 0.05, 0.1,   |
|              | bounds:\                | 0.35\]       |
|              | left = w–x min, right = |              |
|              | y–z min from apex (see  |              |
|              | below).                 |              |
+--------------+-------------------------+--------------+
| num_t hreads | Number of CPU threads   | 2            |
|              | to be used for          |              |
|              | integration task.       |              |
+--------------+-------------------------+--------------+
...
|              | in processing or        |              |
|              | complex ch romatograms. |              |
+--------------+-------------------------+--------------+
| RT \_shift   | \[x, y\] indicate that  | \[-0.2,      |
|              | RT drift is at most x   | 0.2\]        |
|              | minutes left and y      |              |
|              | minutes right of the    |              |
|              | peak in the reference   |              |
|              | samples                 |              |
+--------------+-------------------------+--------------+
| RT\_         | Maximum allowed RT      | 0.1          |
| shift_bound  | drift between adjacent  |              |
|              | samples                 |              |
+--------------+-------------------------+--------------+
...
| Column       | Description                                     |
+==============+=================================================+
| **file       | The mzML file names to be processed, which are  |
| name**       | in the folder defined under \`m zML_files\` in  |
|              | the param.txt file. Must include the .mzML file |
|              | extension.                                      |
+--------------+-------------------------------------------------+
...
|              | differently (see below). For all other cases,   |
|              | any sample type may be defined, and the         |
|              | information will be forwarded to the            |
|              | integration results. The field may also be left |
|              | empty. Use of the default MRMhub QC types (see  |
|              | [MRMhub Data Identifiers](#0)) is recommended   |
...
|              | ensure they exhibit negligible RT shifts        |
...
|              | peak. Multiple distinct features can be defined   |
|              | for a transition.                                 |
+--------------+---------------------------------------------------+
...
|              | orresponding compound. This value has currently   |
|              | no function in the INTEGRATOR modules, but will   |
|              | be included in the results and will be read in by |
|              | the QUANT module. Can be left empty, but if       |
|              | defined must correspond to a defined Compound     |
|              | Name.                                             |
+--------------+---------------------------------------------------+
| **Precursor  | The m/z value of the precursor ion. Must match    |
| Ion**        | the c orresponding value in the mzML file within  |
|              | the tolerance given via `mz_tol` in the param.txt |
|              | file.                                             |
+--------------+---------------------------------------------------+
| **Product    | The m/z value of the product ion. Must match the  |
| Ion**        | c orresponding value in the mzML file within the  |
|              | tolerance given via `mz_tol` in the param.txt     |
|              | file.                                             |
+--------------+---------------------------------------------------+
...
| iform_width  | boundaries are set to the median of the results   |
|              | from the automatic border d e termination.        |
+--------------+---------------------------------------------------+
