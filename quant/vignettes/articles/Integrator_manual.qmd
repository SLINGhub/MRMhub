...
For Windows computers, right-click the executable, select
...
+=============+============================+===============+
| mzML_files  | Location of mzML files to  | mzML          |
|             | be used for peak           |               |
|             | integration                |               |
+-------------+----------------------------+---------------+
| batch_info  | Input file with mzML file  | b             |
|             | names and meta information | atch_info.csv |
|             | (see below)                |               |
+-------------+----------------------------+---------------+
| tran        | Input file with MRM        | fea           |
| sition_list | transition information     | ture_info.csv |
|             | (see below)                |               |
+-------------+----------------------------+---------------+
| peak_width  | \[w, x, y, z\] define      | \[0.17, 0.05, |
|             | allowed integration        | 0.1, 0.35\]   |
|             | bounds:\                   |               |
|             | left = w–x min, right =    |               |
|             | y–z min from apex (see     |               |
|             | below).                    |               |
+-------------+----------------------------+---------------+
| num_t       | Number of CPU threads to   | 2             |
| hreads      | be used for integration    |               |
|             | task.                      |               |
+-------------+----------------------------+---------------+
| mz_tol      | Tolerance between          | 0.06          |
|             | indicated m/z and detected |               |
|             | m/z values                 |               |
+-------------+----------------------------+---------------+
| RT_tol      | Tolerance between          | 0.1           |
|             | indicated RT and detected  |               |
|             | RT. Note\*\*: This is a    |               |
|             | key setting in processing  |               |
|             | or complex ch romatograms. |               |
+-------------+----------------------------+---------------+
| RT \_shift  | \[x, y\] indicate that RT  | \[-0.2, 0.2\] |
|             | drift is at most x minutes |               |
|             | left and y minutes right   |               |
|             | of the peak in the         |               |
|             | reference samples          |               |
+-------------+----------------------------+---------------+
| RT_         | Maximum allowed RT drift   | 0.1           |
| shift_bound | between adjacent samples   |               |
+-------------+----------------------------+---------------+
...
| Column     | Description                                        |
+============+====================================================+
| **file     | The mzML file names to be processed, which are in  |
| name**     | the folder defined under \`m zML_files\` in the    |
|            | param.txt file. Must include the .mzML file        |
|            | extension.                                         |
+------------+----------------------------------------------------+
| batch      | The analysis batch identifier to which the file    |
|            | belong to. This value has no function in the       |
|            | INTEGRATOR, but the value is included in the       |
|            | result.                                            |
+------------+----------------------------------------------------+
| s          | The sample (QC) type of the co rresponding sample. |
| ample_type | Blanks must be labeled with text containing        |
|            | *`BLK`*, as these are processed differently (see   |
|            | below). For all other cases, any sample type may   |
|            | be defined, and the information will be forwarded  |
|            | to the integration results. The field may also be  |
|            | left empty. Use of the default MRMhub QC types     |
|            | (see [MRMhub Data Identifiers](#0)) is recommended |
|            | to ensure seamless integration with the QUANT      |
|            | module.                                            |
+------------+----------------------------------------------------+
| reference  | Indicates that this sample will be used as         |
|            | reference for RT shift estimation (see below).     |
|            | Label with `x`. If selecting multiple samples,     |
|            | ensure they exhibit negligible RT shifts relative  |
|            | to one another.                                    |
+------------+----------------------------------------------------+
...
| Column      | Description                                          |
+=============+======================================================+
| **Compound  | Feature (analyte) identifier name. Must be a unique  |
| Name**      | value, identifying typically a single peak. Multiple |
|             | distinct features can be defined for a transition.   |
+-------------+------------------------------------------------------+
| *           | Transition identifier. Can be repeated values.       |
| *Transition |                                                      |
| Name**      |                                                      |
+-------------+------------------------------------------------------+
| ISTD        | The internal standard feature assigned to the c      |
|             | orresponding compound. This value has currently no   |
|             | function in the INTEGRATOR modules, but will be      |
|             | included in the results and will be read in by the   |
|             | QUANT module. Can be left empty, but if defined must |
|             | correspond to a defined Compound Name.               |
+-------------+------------------------------------------------------+
| **Precursor | The m/z value of the precursor ion. Must match the c |
| Ion**       | orresponding value in the mzML file within the       |
|             | tolerance given via `mz_tol` in the param.txt file.  |
+-------------+------------------------------------------------------+
| **Product   | The m/z value of the product ion. Must match the c   |
| Ion**       | orresponding value in the mzML file within the       |
|             | tolerance given via `mz_tol` in the param.txt file.  |
+-------------+------------------------------------------------------+
| **RT**      | The expected retention time of the c orresponding    |
|             | feature, given in minutes.                           |
+-------------+------------------------------------------------------+
| u           | Either `y` or `n` (default). If yes, the peak        |
| n           | boundaries are set to the median of the results from |
| iform_width | the automatic border d e termination.                |
+-------------+------------------------------------------------------+
| left        | Sets fixed left peak border for all samples, in      |
| integration | minutes (with respect to the reference sample)       |
| bound       |                                                      |
+-------------+------------------------------------------------------+
| right       | Sets fixed right peak border for all samples, in     |
| integration | minutes (with respect to the reference sample)       |
| bound       |                                                      |
+-------------+------------------------------------------------------+
| peak width  | \[w, x, y, z\] define allowed integration bounds:    |
|             | left = w–x min, right = y–z min from apex (see       |
|             | below).\                                             |
|             | Note: This overwrites the globally defined peak      |
|             | width (param.txt). See below.                        |
+-------------+------------------------------------------------------+
