...
raw data files (.wiff for SCIEX, .raw files for Thermo Fisher,
...
<https://github.com/SLINGhub/MRMhub/releases>. Unzip the downloaded
...
set of input and raw data files, which can be used as templates for future analysis. 
The `MRMhub_plot.r` is an R script used to generate the PDFs. 
It can be modified to tailor for specific layouts and design features of the plots.
...
This file defines data files. The file names must be presented in their processing order
from top to bottom. mzML files present in the data folder but not listed here will be
ignored. Columns containing required information for individual data files are
marked in **bold**.
...
|               | with `x`. If selecting multiple   |
|               | samples, ensure they exhibit      |
|               | negligible RT shifts relative to  |
|               | one another.                      |
+---------------+-----------------------------------+
...
|               | file within the tolerance given |
...
|               | given in minutes.               |
+---------------+---------------------------------+
| uniform_width | Either `y` or `n` (default).    |
...
-   **`missing_compounds.txt`** reports transitions defined in the
    ...
    processed.
-   `missing_details.txt` This report details the missing transitions and identifies exactly which samples lack them. This is useful when a transition appears in some samples but not others.
...
-   `by_transition_low`: Contains PDFs of transitions where none of the
    ...
    intensity* threshold defined by the constant `low_intensity_threshold` in the provided
    `MRMhub_plot.r` R script.
-   `by_sample`: Provides a PDF for each reference sample; all of the sample’s transitions are plotted within the same file.

For efficient viewing of PDFs on macOS, open the PDF folder, press *Space*
