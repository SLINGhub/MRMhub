...
post processing of data. MRMhub therefore inspects imported
data/metadata for completeness of data and consistency of IDs used
across differet metadata tables. After importing metadata a summary of
identified error, warnings and notes concerning metadata is printed to
the console, to allow user to identify and adress possible issues.

### Metadata formats and templates

The structure and required/optional columns for each metadata type is
descrived in the Manual and in the help of the corresponding import
functions.

To obtain templates for metadata, an Excel file containing all template
...
this case we explicitly do not import any metadata present in the
...
mexp <- import_data_mrmhub(data = mexp, path = data_path, import_metadata = TRUE)
```
...
an Excel file (`.xlsx`). This template offers tables for all metadata
types supported by MRMhub, with options to do perform checks on
validatiy and integrity of the metadata.

The template can be obtained from <https://github.com/SLINGhub/mrmhub>
or via a mrmhub function
