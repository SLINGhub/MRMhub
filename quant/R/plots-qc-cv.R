...
plot_normalization_qc <- function(
  ...
  if (missing(plot_type)) {
    cli::cli_abort(
      "{.arg plot_type} must be supplied ('scatter', 'diff', or 'ratio')."
    )
  }

  if (missing(before_norm_var) || missing(after_norm_var)) {
    cli::cli_abort(
      "{.arg before_norm_var} and {.arg after_norm_var} must be supplied."
    )
  }
...
plot_qcmetrics_comparison <- function(
  ...
  if (!all(is.na(threshold_values))) {
    if (length(threshold_values) == 1) {
      threshold_values <- rep(threshold_values, 2)
    }
