...
#' @description Imports metadata from a 'MRMhub Metadata Organizer' file (.xlsm/.xlsx) file and associates it with analysis data.
#' @param data A `MRMhubExperiment` object
#' @param path File name and path of the 'MRMhub Metadata Organizer' file (.xlsm/.xlsx) file
...
print_assertion_summary <- function(
  ...
  if (!is.null(t1)) {
    t1 <- t1 |>
      mutate(ignore_warn_flag = attr(data$annot_analyses, "ignore_warnings"))
    if (
      attr(data$annot_analyses, "excl_unmatched_analyses") &
...
read_metadata_msorganiser <- function(path, trim_ws = TRUE) {
  ...
  if (!(fs::path_ext(path) %in% c("xlsm", "xlsx"))) {
    cli::cli_abort(col_red(
      "Invalid file type not. A MSOrganiser template file (*.xlsm or *.xlsx) must be provided."
    ))
  } else {
