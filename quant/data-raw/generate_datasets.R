...
# Create an unprocessed lipidomics dataset with data and metadata as a MRMhubExperiment object

lipidomics_dataset <- MRMhubExperiment(analysis_type = "lipidomics")

lipidomics_dataset <- import_data_mrmhub(
  data = lipidomics_dataset,
  path = here("data-raw/MRMhub_example.tsv"),
  import_metadata = T
)
...
save_dataset_csv(
  data = lipidomics_dataset,
  path = here("data-raw/MRMhub_example.csv"),
  variable = "intensity",
  filter_data = FALSE,
  add_qctype = TRUE,
  qc_types = c("SPL", "BQC")
...
# with data and metadata as a MRMhubExperiment object
