...
test_that("check_integrity_analyses works", {
  ...
  expect_error(
    check_integrity_analyses(
      mexp_temp,
      excl_unmatched_analyses = FALSE,
      silent = FALSE
    ),
    "No metadata present for 2 of 499 analyses: Longit_B-ISTD 01 Extr, Longit_TQC-100%",
    fixed = TRUE
  )

  expect_no_error(check_integrity_analyses(
    mexp_temp,
    excl_unmatched_analyses = FALSE,
    silent = TRUE
  ))
  ...
  expect_true(check_integrity_analyses(
    mexp_temp,
    excl_unmatched_analyses = TRUE,
    silent = TRUE
  ))
  expect_false(check_integrity_analyses(
    mexp_temp,
    excl_unmatched_analyses = FALSE,
    silent = TRUE
  ))
  ...
  mexp_temp <- mexp
  mexp_temp@dataset_orig <- mexp_temp@dataset_orig[
    mexp_temp@dataset_orig$analysis_id != "Longit_B-ISTD 01 Extr" &
