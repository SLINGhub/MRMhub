...
test_that("Imports/associates data and metadata, orders analyses by dataset (timestamp missing)", {
  ...
  mexp <- mrmhub::import_metadata_msorganiser(
    mexp,
    path = testthat::test_path(
      "testdata/MRMhub_Metadata_Template_191_20240226_MHQuant_S1P_V1.xlsm"
    ),
...
test_that("import_metadata_msorganiser handles missing / invalid files", {
  ...
  expect_error(
    mexp <- mrmhub::import_metadata_msorganiser(
      mexp,
      path = testthat::test_path(
        "testdata/MRMhub_Metadata_Template_wrongabout.xlsm"
      ),
      excl_unmatched_analyses = FALSE
    ),
  ...
  expect_error(
    mexp <- mrmhub::import_metadata_msorganiser(
      mexp,
      path = testthat::test_path(
        "testdata/MRMhub_Metadata_Template_missingabout.xlsm"
      ),
      excl_unmatched_analyses = FALSE
    ),
  ...
  expect_error(
    mexp <- mrmhub::import_metadata_msorganiser(
      mexp,
      path = testthat::test_path(
        "testdata/MRMhub_Metadata_Template_lowerversion.xlsm"
      ),
      excl_unmatched_analyses = FALSE
    ),
  ...
  expect_error(
    mexp <- mrmhub::import_metadata_msorganiser(
      mexp,
      path = testthat::test_path(
        "testdata/MRMhub_Metadata_Template_higherversion.xlsm"
      ),
      excl_unmatched_analyses = FALSE
    ),
    "Unsupported MSOrganiser template version. Please use an MSOrganiser template v0.2 or higher.",
  ...
  expect_error(
    mexp <- mrmhub::import_metadata_msorganiser(
      mexp,
      path = testthat::test_path(
        "testdata/MRMhub_Metadata_Template_invalidversion.xlsm"
      ),
      excl_unmatched_analyses = FALSE
    ),
...
test_that("Raise data assertion warning and stops with not all analyses defined in analysis metadata ", {
  ...
  expect_error(
    mexp <- mrmhub::import_metadata_msorganiser(
      mexp,
      path = testthat::test_path(
        "MRMhub_Metadata_Template_191_20240226_MHQuant_S1P_analysissubset.xlsm"
      ),
      excl_unmatched_analyses = FALSE
    ),
