...
function createRenderQueue() {
  ...
  const finishIfIdle = () => {
    if (streamDone && !scheduled && cursor >= queue.length) resolveDrain();
  };
  ...
  const pump = () => {
    ...
    while (
      ...
      cursor += 1;
      gen_svg(job.message, job.id, job.index, job.container);
      count += 1;
    }
...
async function gen(trans, mzml_l) {
  ...
  let ii = 0;
  onEvent.onmessage = (message) => {
    queue.push({ message, id: mzml_l[ii], index: ii, container });
    ii++;
  };
...
function gen_svg({ sh, pos_l, bl, te }, id, ii, container) {
  ...
  const visibleDots = te
    .map((point, index) => [point, index])
    .filter(([point]) => point.y <= y.domain()[1]);
  const dotStep = Math.max(1, Math.ceil(visibleDots.length / maxHoverDots));
  const dotData = visibleDots.filter((_, index) => index % dotStep === 0);
  const dotLayer = svg.append("g").style("display", "none");

  const tt = svg.append("g").style("display", "none");
  const ttNode = tt.node();
  const ttCircle = tt.append("circle").attr("r", 3).node();
  tt.append("rect")
    .attr("width", 55)
    .attr("height", marginTop + 4)
    .attr("x", -tt.select("rect").attr("width") / 2)
    .attr("rx", 4);
  const ttText = tt.append("text")
    .attr("text-anchor", "middle")
    .attr("dominant-baseline", "text-before-edge")
    .attr("fill", "white")
    .node();
  ...
  const hideDots = () => {
    ...
    dotLayer.style("display", "none").selectAll("circle").remove();
  };
  const hideGuides = () => {
    for (const target of traceGraphs) {
      target.guide.style.display = "none";
    }
  };
  ...
  svg
    .on("pointerenter pointermove", (event) => {
      ...
      ttText.textContent = formatRt(te[i].x);
      const cx = x(te[i].x);
      ttNode.setAttribute("transform", `translate(${cx})`);
      ttNode.style.display = "";
      ttCircle.setAttribute("cy", String(y(te[i].y)));
      const adj_chrom = id[4] ? 50 : 0.1;
      for (const target of traceGraphs) {
        if (Math.abs(target.index - ii) >= adj_chrom) continue;
        target.guide.style.display = "";
        target.guide.setAttribute("x1", String(cx));
        target.guide.setAttribute("x2", String(cx));
      }
    })
    .on("pointerleave", () => {
      hideDots();
