...
- **R:** the current release, from [CRAN](https://cran.r-project.org). Minimum
  is 4.1; on older releases some packages must be built from source, which is
  slower and requires additional tools.
...
`package 'mrmhub' was built under R version ...` appears. It can be ignored:
`mrmhub` contains no compiled code, so the version difference has no effect.
If the package does not load, see [Troubleshooting](#troubleshooting) or the
ZIP-file method below. For optional features, see
[Optional packages](#optional-packages).

### Alternative installation methods

If R-universe is unreachable, MRMhub can be installed from GitHub. Both routes
...
`pak` may fail because of a network or proxy restriction, or a request for
build tools such as the Xcode command line tools on macOS. In that case, use
`remotes`:
...
If the methods above fail or R cannot reach the repositories, download a copy of
