...
3. **Add the input templates** — the template `param.txt`, `run_order.csv`, and `feature_list.csv`
   bundled in a [release](https://github.com/SLINGhub/MRMhub/releases), or copied from an existing project.
...
Edit and review the input files that configure the peak integration run — one global, and two defining the samples and the features. See the [Input Files reference](input-files.html) for a description of all parameters.
...
[Processing Workflow](running.html) for the full reference. It is started by double-clicking `MRMhub`
...
**Hit a problem?** The [FAQ & Troubleshooting](faq.html) page covers the most frequent failure
modes — a misread CSV, an unmatched transition, a zero area, or MRMhub-viz not loading.
...
- **`quant_raw.csv`** (wide format) — a features × samples matrix of **peak areas** only, with the
  precursor and product *m/z* and the expected RT (from `feature_list.csv`) carried in the top rows.
...
See [Processing Workflow](running.html#step-3-peak-integration) for the full definition of these file formats.

## 7. Post-processing

One recommended route is **MRMhub QUANT**, the
post-processing module of MRMhub. This R package imports INTEGRATOR's `long.csv` directly for normalisation, drift and batch correction, calibration, quality control, and
reporting:
