...
get_summary_stats <- function(
  ...
  if (min_groupsize <= 2)
    cli::cli_abort("group size must be >= 2 for the statistical tests")
  ...
  d_filt <- data |>
    ...
    mutate(
      !!grouping := forcats::fct_relevel(!!grouping, c(group_case, group_ref))
    )
  ...
  d_incompletegroups <- d_filt |>
    ...
    summarise(
      grp_size_fail = sum(
        !is.na(.data$feature_conc[!!grouping == group_case])
      ) <
        min_groupsize |
  ...
  if (nrow(d_incompletegroups) > 0) {
    warning(glue::glue(
      "Following features were ignored as they have group size(s) of less than {min_groupsize}:\n {incomplete}"
    ))
  }
  ...
  if (!paired) {
    d_fc <- d_filt |>
      ...
      summarise(
        log2FC = log2(
          mean(.data$feature_conc[!!grouping == group_case]) /
            mean(.data$feature_conc[!!grouping == group_ref])
        )
      )
  } else {
    d_fc <- d_filt |>
      ...
      summarise(
        log2FC = mean(log2(
          .data$feature_conc[!!grouping == group_case] /
            .data$feature_conc[!!grouping == group_ref]
        ))
      )
  }
