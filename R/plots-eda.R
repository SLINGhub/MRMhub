...
volcano_plot <- function(
  ...
  d_FC$Significant <- ifelse(
    (d_FC$p_value_mod < sig_p_value_min) &
      (abs(d_FC$log2FC) > log2(sig_FC_min)),
    "sign",
    "ns"
  )
  ...
  p <- ggplot(d_FC, aes(x = .data$log2FC, y = -log10(.data$p_value_mod))) +
    ...
    geom_point(
      ...
      alpha = .data$point_transparency,
      na.rm = T,
      aes(colour = .data$Significant)
    ) +
  ...
  if (symmetric_x) {
    FC_seq <- seq(
      from = -(ceiling(FC_abs_max)),
      to = ceiling(FC_abs_max),
      by = axistick
    )
    # print(FC_seq)
    p <- p +
      scale_x_continuous(
        limits = c(-(FC_abs_max * 1.1), FC_abs_max * 1.1),
        breaks = FC_seq,
        expand = c(0.02, 0.02)
      )
  } else {
...
add_lipid_transition_classnames <- function(datLong) {
  ...
  datLong_temp <- datLong |>
    mutate(
      lipidClassBase = (str_trim(str_extract(
        .data$Compound,
        "[A-z0-9]+[[:blank:]]*"
      )))
    )
  datLong_temp <- datLong_temp |>
    mutate(
      lipidClass = (str_trim(str_extract(
        .data$Compound,
        "[A-z0-9]+[[:blank:]]*([A-Z]{1}|[d|t|m])"
      )))
    )
  ...
  datLong_temp <- datLong_temp |>
    mutate(
      lipidClassSL = str_extract(
        .data$Compound,
        "[A-z0-9]+[[:blank:]]*([A-Z]{1}|[d|t|m][0-9\\:]{4})"
      )
    )
  # Add  transition name  (defined by flanking "[]")
  datLong_temp <- datLong_temp |>
    mutate(transitionName = str_extract(.data$Compound, "(?<=\\[).*?(?=\\])"))
  datLong_temp <- datLong_temp |>
    mutate(
      transitionName = ifelse(
        is.na(.data$transitionName),
        "",
        .data$transitionName
      )
    )
  ...
  datLong_temp <- datLong_temp |>
    mutate(
      transitionClass = ifelse(
        grepl("\\[\\-|\\[NL", .data$Compound),
        "M>M-NL",
        str_replace(
          paste(
            "",
            str_trim(str_extract(.data$Compound, "\\[[a-zA-Z0-9\\-\\: ]*\\]"))
          ),
          " NA",
          .data$transitionName
        )
  ...
  datLong_temp <- datLong_temp |>
    mutate(CompoundName = get_CompoundName(.data$Compound))
...
plot_heatmap <- function(
  ...
  z_score_seq <-
    seq(
      ceiling(min(t(m_scaled), na.rm = T)),
      ceiling(max(t(m_scaled), na.rm = T)),
      by = 1
    )
  ...
  df_compounds <- data.frame(Compound = colnames(m_scaled)) |>
...
plot_pca_sling2 <- function(
  ...
  if (
    !all(
      d_filt |> pull(.data$analysis_id) == d_metadata |> pull(.data$analysis_id)
    )
  )
    cli::cli_abort("Data and Metadata missmatch")
    ...
    if (!mark_ellipse) {
      ...
      p <- p +
        ggforce::geom_mark_ellipse(
          aes(
            fill = !!rlang::ensym(grouping),
            color = !!rlang::ensym(grouping),
            label = ""
          ),
          alpha = fill_alpha,
          label.colour = c("white", "white"),
          con.size = 0,
          linewidth = 0.01
        )
    }
  ...
  p <- p +
    ...
    xlab(glue::glue(
      "PC{dim_x} ({round(pca_contrib[[dim_x,'percent']]*100,1)}%)"
    )) +
    ylab(glue::glue(
      "PC{dim_y} ({round(pca_contrib[[dim_y,'percent']]*100,1)}%)"
    )) +
    theme(
...
get_pval <- function(df, test_col, contrasts, paired) {
  ...
  map_dfr(
    ...
    .f = ~ tibble::tibble(
      p.value = mod_t_test(
        formula = feature_conc ~ factor(`_group`),
        paired = paired,
        data = subset(df, `_group` %in% .x)
      )$p.value
    ) |>
      mutate(
        ...
        grp1 = .x[1],
        grp2 = .x[2],
        y_max_1 = mean(df$feature_conc[df$`_group` == .x[1]]) +
          sd(df$feature_conc[df$`_group` == .x[1]]),
        y_max_2 = mean(df$feature_conc[df$`_group` == .x[2]]) +
          sd(df$feature_conc[df$`_group` == .x[2]]),
        y_max = max(y_max_1, y_max_2)
      )
  )
}

plot_dotboxplus_onepage <- function(
  data,
  outer_inner,
  GroupOuter = NULL,
  GroupInner,
  ...
  if (outer_inner) {
    ...
    d_stat <- data |>
      ...
      mutate(
        x_min = (.data$group_outer_id - 0.4) +
          0.8 / n_groups / 2 +
          0.8 * 1 / (n_groups) * (match(.data$grp1, group_levels) - 1),
        x_max = (.data$group_outer_id - 0.4) +
          0.8 / n_groups / 2 +
          0.8 * 1 / (n_groups) * (match(.data$grp2, group_levels) - 1),
    ...
    plt <- plt +
      ...
      geom_boxplot(
        width = 0.7,
        aes(color = {{ GroupInner }}, fill = {{ GroupInner }}),
        alpha = 0.1,
        lwd = 0.03,
        outlier.shape = NA,
...
plot_dotboxplus <- function(
  ...
  page_list <- df |>
    ...
    mutate(
      lipid_class_page_no = max(ceiling(
        which(unique(.data$feature_id) == .data$feature_id) /
          (rows_page * cols_page)
      ))
    ) |>
    group_by(.data$feature_id) |>
  ...
  if (class_per_page) {
    plt_list <- page_list |>
      ...
      mutate(
        plt = map(
          data,
          ~ plot_dotboxplus_onepage(
            .,
            outer_inner = FALSE,
            GroupOuter = {{ outer_group }},
            GroupInner = {{ inner_group }},
            n_row = rows_page,
            n_col = cols_page,
            scale_colors = inner_group_colors,
            contrasts = contrasts,
            paired = paired,
            scale_text,
            scale_signf,
