...
#'   (e.g., batch ID, sample type) into the `MRMhubExperiment` object. Supported metadata columns are:
...
#' CSV column names to the internal column names used by `MRMhubExperiment`.
...
# TODO: remove support for norm intensity
parse_mrmhub_result <- function(path, silent = FALSE) {
  col_map <- c(
...
#' #' @param sheet Sheet name in case an Excel file (.xls, .xlsx) is imported
#' #' @param silent Suppress messages
#'
#' #' @return A tibble in the long format
#' #' @noRd
...
# # for MRMhub wide (previous) result format
# parse_mrmhub_result_wide <- function(path, use_normalized_data = FALSE, silent = FALSE) {
#
#   ext_file <- tolower(fs::path_ext(path))
#
#   sep <- case_when(
...
#   )
#
#   if (is.na(sep)) cli::cli_abort("Data file type/extension not supported. Please re-export results from MRMhub.")
#
#   d_mrmhub_raw <- readr::read_delim(path,
#                                     delim = sep, col_types = readr::cols(.default = "c"),
#                                     col_names = TRUE, trim_ws = TRUE
#   )
#
#
#   # Extract MRMhub's "QC" info
#   d_mrmhub_featureinfo <- d_mrmhub_raw |>
...
#     dplyr::left_join(d_mrmhub_featureinfo, by = "feature_id") |>
#     dplyr::relocate(.data$value, .after = dplyr::last_col()) |>
#     dplyr::mutate(dplyr::across(where(is.character), stringr::str_squish))
#
#   if (use_normalized_data) {
#     d_mrmhub_data <- d_mrmhub_data |>
...
#   }
#
#   names(d_mrmhub_data) <- tolower(names(d_mrmhub_data))
#
...
#
#   if (!silent) {
#     cli_alert_success(cli::col_green("Imported {length(unique(d_mrmhub_data$raw_data_filename))} analyses with {length(unique(d_mrmhub_data$feature_id))} features \n"))
#   }
#
#   d_mrmhub_data
# }
