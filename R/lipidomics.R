...
#' @return The input data frame with added lipid class, species and transition name columns.
...
parse_lipid_feature_names <- function(
  ...
  dat_temp <- dat_temp |>
    mutate(
      feature_id = str_squish(str_replace(
        .data$feature_id,
        "\\s*\\(\\-H2O\\)?\\s*",
        " [M-H2O]"
      ))
  ...
  dat_temp <- dat_temp |>
    mutate(
      transition_name = str_extract(.data$feature_id, "(?<=\\[).*?(?=\\])")
    )
  dat_temp <- dat_temp |>
    mutate(
      transition_name = ifelse(
  ...
  dat_temp <- dat_temp |>
    mutate(
      transition_class = ifelse(
        grepl("\\[\\-|\\[NL", .data$feature_id),
        "M>M-NL",
        str_replace(
          paste(
            "",
            str_trim(str_extract(.data$feature_id, "\\[[a-zA-Z0-9\\-\\: ]*\\]"))
          ),
          " NA",
          .data$transition_name
        )
  ...
  dat_temp_rgoslin <- dat_temp |>
    mutate(
      lipid_name = str_trim(str_replace(.data$analyte_id, "\\[.*?\\]", ""))
    ) |>
    mutate(lipid_name = str_replace(.data$lipid_name, "\\s*\\(.*?\\)", "")) |>
    mutate(lipid_name = str_replace(.data$lipid_name, "\\s*/\\s*", "/")) |>
    mutate(
      lipid_name = str_replace(.data$lipid_name, "^(\\S+\\s+\\S+).*", "\\1")
    ) #Remove all text after 2nd space
  d_goslin <- suppressMessages(rgoslin::parseLipidNames(
    dat_temp_rgoslin$lipid_name
  ))

  goslin_err <- d_goslin |>
    filter(.data$Grammar == "NOT_PARSEABLE") |>
    pull("Original.Name")

  goslin_err_features <- dat_temp_rgoslin |>
    filter(.data$lipid_name %in% goslin_err) |>
    pull("feature_id")
  if (length(goslin_err_features) > 0) {
    mh_warn(
      "Lipid names could not be parsed for {length(goslin_err_features)} of {nrow(dat_temp)} feature{?s}: {.val {mh_vec(goslin_err_features)}}."
    )
  }
  ...
  d_goslin <- d_goslin |>
    ...
    mutate(
      Normalized.Name = if_else(
        str_detect(.data$Original.Name, " \\-P| P\\-"),
        str_replace(.data$Normalized.Name, " O\\-", " P-"),
    ...
    mutate(
      lipid_class_lcb = if_else(
        .data$Lipid.Maps.Category == "SP" &
          str_detect(.data$Normalized.Name, ";O\\d?"),
        stringr::str_c(
          .data$Extended.Species.Name,
          str_extract(.data$Normalized.Name, ";O\\d?")
        ),
        .data$lipid_class_lcb
      )
    ) |>
    mutate(
      lipid_class_lcb = if_else(
        .data$Extended.Species.Name == "TG" &
          str_detect(.data$Normalized.Name, "TG O\\-"),
        "TG-O",
        .data$lipid_class_lcb
      )
    ) |>
    mutate(
      lipid_class_lcb = if_else(
        .data$Extended.Species.Name == "DG" &
          str_detect(.data$Normalized.Name, "DG O\\-"),
        "DG-O",
        .data$lipid_class_lcb
      )
    ) |>
    mutate(
      lipid_class_lcb = if_else(
        .data$Extended.Species.Name == "SPBP",
        "SPBP",
        .data$lipid_class_lcb
      )
    ) |>
    mutate(
      lipid_class_lcb = if_else(
        .data$Extended.Species.Name == "ST 27:1",
        "CE",
        .data$lipid_class_lcb
      )
    ) |>
    mutate(
      lipid_class_lcb = if_else(
        .data$Extended.Species.Name == "ST 27:1;O",
        "Chol",
        .data$lipid_class_lcb
      )
    ) |>
    ungroup() |>
    select(
      analyte_name = "Normalized.Name",
      lipid_class = "Extended.Species.Name",
      "lipid_class_lcb",
      lipid_class_base = "Lipid.Maps.Category",
      "chem_formula" = "Sum.Formula",
      total_c = "Total.C",
      total_db = "Total.DB"
    )
  ...
  dat_temp <- dat_temp |>
    ...
    mutate(
      transition_group_id = match(
        .data$transition_name,
        unique(.data$transition_name[
          .data$lipid_class_lcb == dplyr::first(.data$lipid_class_lcb)
        ])
      ),
      transition_group_id = if_else(
        is.na(.data$transition_group_id),
        1L,
        .data$transition_group_id
      )
    ) |>
    ungroup()

  if (!add_transition_names) {
    dat_temp <- dat_temp |>
      select(!any_of(c("transition_group_id", "transition_name")))
  }
  if (!add_chain_composition) {
    dat_temp <- dat_temp |> select(!any_of(c("total_c", "total_db")))
  }
...
convert_triglycerides <- function(dat) {
  d_tg <- dat |>
    ...
    tidyr::separate(
      .data$new_nl_name,
      into = c("class", "sum_chain", "nl_chain", "isomer"),
      sep = " ",
      remove = FALSE,
      fill = "right"
    ) |>
    ...
    mutate(
      isomer = if_else(
        !str_detect(.data$isomer_temp, "\\[NL-|\\[\\-"),
        .data$isomer_temp,
        NA
      )
    ) |>
    ...
    tidyr::separate(
      .data$sum_chain,
      into = c("sum_chain_c", "sum_chain_db"),
      sep = ":",
      remove = FALSE,
      convert = TRUE
    ) |>
    tidyr::separate(
      .data$nl_chain,
      into = c("nl_chain_c", "nl_chain_db"),
      sep = ":",
      remove = FALSE,
      convert = TRUE
    ) |>
    mutate(
      second_c = .data$sum_chain_c - .data$nl_chain_c,
      second_db = .data$sum_chain_db - .data$nl_chain_db
    ) |>
    mutate(
      new_nl_name = str_squish(glue::glue(
        "{class} {nl_chain_c}:{nl_chain_db}_{second_c}:{second_db} {isomer}"
      ))
    ) |>
  ...
  d_lyso <- dat |>
    filter(
      str_detect(.data$analyte_id, "^LPC|^LPE|^LPI|^LPS") &
        !str_detect(.data$analyte_id, "O\\-|P\\-|\\-O|\\-P")
    ) |>
    mutate(
      analyte_id_temp = str_replace(
        .data$analyte_id,
        "(?i)(\\[sn1\\] |-sn1)",
        " (sn1)"
      )
    ...
    tidyr::separate(
      .data$analyte_id_temp,
      into = c("class", "chain", "isomer1", "isomer2"),
      fill = "right",
      extra = "drop",
      sep = " ",
      remove = FALSE
    ...
    mutate(
      analyte_new = case_when(
        (.data$isomer1 == "a" | .data$isomer1 == "sn2") &
          !is.na(.data$isomer1) ~
          glue::glue("{class} 0:0/{chain} ({isomer2})"),
        (.data$isomer1 == "b" | .data$isomer1 == "sn1") &
          !is.na(.data$isomer1) ~
          glue::glue("{class} {chain}/0:0 ({isomer2})"),
...
normalize_isotope_labels <- function(x) {
  ...
  sapply(seq_along(matches), function(i) {
    ...
    after_core <- paste(
      stringr::str_extract_all(cleaned, "\\[.*?\\]")[[1]],
      collapse = ""
    )
