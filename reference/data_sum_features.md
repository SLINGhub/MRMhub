...
- feature_classes:
  ...
  Select feature classes to sum up. Default (NA) is to sum up all
  feature classes according to their analyte_id. to include them in the
  sum if quantifier select "include", to not sum them up select
  "exclude".
